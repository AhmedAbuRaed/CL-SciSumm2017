...
/**
 * Created by ahmed on 6/24/2017.
 */
public class ContextVectors {
    ...
    public static HashMap<String, Document> run(HashMap<String, Document> rawDocuments, String workingDirectory) {
        try {
            // load the GAPP
            application = (CorpusController) PersistenceManager.loadObjectFromFile(new File(workingDirectory + File.separator + "contextVectorsCorpusPipelineApp.gapp"));

            Corpus corpus = null;
            for (String key : rawDocuments.keySet()) {
                System.out.println("Generating Context Vectors for Document: " + key);
