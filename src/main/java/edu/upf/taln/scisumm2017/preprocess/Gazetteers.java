...
public class Gazetteers {
    ...
    public static HashMap<String, Document> run(HashMap<String, Document> rawDocuments, String workingDirectory) {
        try {
            // load the GAPP
            application = (CorpusController) PersistenceManager.loadObjectFromFile(new File(workingDirectory + File.separator + "gazetteerCorpusPipelineApp.gapp"));

            Corpus corpus = null;
            for (String key : rawDocuments.keySet()) {
                System.out.println("Generating Gazetteers for Document: " + key);
